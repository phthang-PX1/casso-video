function weightProp(weight, framework) {
    if (weight === 'Outline')
        return '';
    if (framework === 'html')
        return ' weight="filled"';
    return ` weight="${weight}"`;
}
function sizeProp(size, framework) {
    if (framework === 'vue')
        return ` :size="${size}"`;
    if (framework === 'html')
        return ` size="${size}"`;
...
function colorProp(color, framework) {
    if (framework === 'vue')
        return ` color="${color}"`;
    if (framework === 'html')
        return ` color="${color}"`;
...
export function generateCode(icon, input) {
    ...
    const colorAttr = (framework) => hasColor ? colorProp(color, framework) : '';
...
export function buildSvgMarkup(icon, weight) {
    const weightData = icon.weights[weight];
    if (!weightData) {
        return { error: `Icon "${icon.name}" does not have a ${weight} weight.` };
    }
