...
export class DragElement extends Interaction {
    ...
    commitTranslation(dx, dy) {
        ...
        const commands = this.dragItems.map((item) => {
            ...
            return new UpdateElementCommand(item.element, { attributes: { transform } }, { attributes: { transform: originalTransform } });
        });
        if (commands.length) {
    ...
    collectGuideCandidates(selection) {
        ...
        const horizontal = [];
        elements.forEach((el) => {
            const { x, y, width, height } = getElementViewportBounds(svg, el);
    ...
    ensureGuideLine(orientation) {
        ...
        if (existing)
            return existing;
        const line = this.interaction.appendTransientElement(createElement('line', {
            stroke: '#FF7A45',
            'stroke-width': 1,
