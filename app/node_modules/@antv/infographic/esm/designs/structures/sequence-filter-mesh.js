...
function renderParticles(particles, options) {
    return particles.map((particle) => {
        ...
        return (_jsx(Path, { d: "M4 0L8 4L4 8L0 4Z", fill: color, x: particle.x, y: particle.y, width: PARTICLE_CONFIG.SIZE, height: PARTICLE_CONFIG.SIZE, "data-element-type": "shape" }));
    });
}
/**
 * 创建装饰元素
...
function createDecorElement(index, itemX, color, particles, options) {
    return (_jsxs(Group, { x: itemX, y: 0, width: SHAPE_CONFIG.WIDTH, height: SHAPE_CONFIG.HEIGHT, children: [_jsxs(ShapesGroup, { children: [_jsx(Path, { d: createMeshPath(), stroke: "#D9D9D9", strokeWidth: 2 }), _jsx(Path, { d: `M${SHAPE_CONFIG.LINE_X} 25V260`, stroke: "#BFBFBF", strokeWidth: 2, strokeLinecap: "round" }), _jsx(Rect, { x: SHAPE_CONFIG.RECT_X, y: SHAPE_CONFIG.RECT_Y, width: SHAPE_CONFIG.RECT_WIDTH, height: SHAPE_CONFIG.RECT_HEIGHT, fill: "#FFCB0E", fillOpacity: 0.2 })] }), _jsx(_Fragment, { children: renderParticles(particles, options) }), _jsx(Text, { x: SHAPE_CONFIG.LINE_X - 25, y: 0, width: 50, height: 20, fontSize: 20, fontWeight: "bold", alignHorizontal: "center", alignVertical: "top", fill: color, children: String(index + 1).padStart(2, '0') })] }));
}
/**
 * 创建箭头元素
 */
function createArrowElement(itemX, options) {
    ...
    return (_jsxs(Group, { x: itemX + SHAPE_CONFIG.WIDTH, y: arrowY, children: [_jsx(Path, { d: "M0 13.9679H57.1429V0L100 74L57.1429 148V134.032H0V13.9679Z", width: SHAPE_CONFIG.ARROW_WIDTH, height: SHAPE_CONFIG.ARROW_HEIGHT, fill: "#FFCB0E", fillOpacity: 0.2, "data-element-type": "shape" }), _jsx(_Fragment, { children: renderParticles(arrowParticles, options) })] }));
}
export const SequenceFilterMesh = (props) => {
    ...
    const itemBounds = getElementBounds(_jsx(Item, { indexes: [0], data: data, datum: items[0], width: SHAPE_CONFIG.WIDTH, positionH: "center" }));
    const decorElements = [];
    const itemElements = [];
    const btnElements = [];
    ...
    items.forEach((item, index) => {
        ...
        // 数据项
        itemElements.push(_jsx(Item, { indexes: indexes, datum: item, data: data, width: SHAPE_CONFIG.WIDTH, x: itemX + SHAPE_CONFIG.LINE_X - SHAPE_CONFIG.WIDTH / 2, y: itemYPos, positionH: "center" }, `item-${index}`));
        // 删除按钮
        btnElements.push(_jsx(BtnRemove, { indexes: indexes, x: itemX + SHAPE_CONFIG.LINE_X - btnBounds.width / 2, y: btnY }));
        // 中间添加按钮
        if (index < items.length - 1) {
            btnElements.push(_jsx(BtnAdd, { indexes: [index + 1], x: itemX + SHAPE_CONFIG.WIDTH - btnBounds.width / 2, y: btnY }));
        }
    });
    // 首尾添加按钮
    if (items.length > 0) {
        btnElements.unshift(_jsx(BtnAdd, { indexes: [0], x: -btnBounds.width / 2, y: btnY }));
        const lastItemX = (items.length - 1) * SHAPE_CONFIG.WIDTH;
        btnElements.push(_jsx(BtnAdd, { indexes: [items.length], x: lastItemX + SHAPE_CONFIG.WIDTH - btnBounds.width / 2, y: btnY }));
    }
    return (_jsxs(FlexLayout, { id: "infographic-container", flexDirection: "column", justifyContent: "center", alignItems: "center", children: [titleContent, _jsxs(Group, { width: SHAPE_CONFIG.WIDTH * items.length + SHAPE_CONFIG.ARROW_WIDTH, height: SHAPE_CONFIG.HEIGHT + gap + itemBounds.height, children: [_jsx(Group, { children: decorElements }), _jsx(ItemsGroup, { children: itemElements }), _jsx(BtnsGroup, { children: btnElements })] })] }));
};
