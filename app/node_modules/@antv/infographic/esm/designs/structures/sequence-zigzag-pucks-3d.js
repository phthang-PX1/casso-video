...
export const SequenceZigzagPucks3d = (props) => {
    ...
    items.forEach((item, index) => {
        ...
        btnElements.push(_jsx(BtnRemove, { indexes: indexes, x: itemX + itemBounds.width - btnBounds.width / 2, y: itemY + itemBounds.height - btnBounds.height / 2 }));
        if (index === 0) {
            btnElements.push(_jsx(BtnAdd, { indexes: [0], x: puckX + puckWidth / 2 - btnBounds.width / 2 - (gap + puckWidth) / 2, y: puckY + puckHeight / 2 - btnBounds.height / 2 }));
        }
        if (index < items.length - 1) {
            ...
            btnElements.push(_jsx(BtnAdd, { indexes: [items.length], x: puckX + puckWidth + gap / 2 - btnBounds.width / 2, y: puckY + puckHeight / 2 - btnBounds.height / 2 }));
        }
    });
    return (_jsxs(FlexLayout, { id: "infographic-container", flexDirection: "column", justifyContent: "center", alignItems: "center", gap: 30, children: [_jsx(Defs, { children: DropShadowFilter }), titleContent, _jsxs(Group, { x: 0, y: 0, children: [_jsx(Group, { children: puckElements }), _jsx(ItemsGroup, { children: itemElements }), _jsx(BtnsGroup, { children: btnElements })] })] }));
};
