...
function Tree(size) {
    ...
    return (_jsxs(ShapesGroup, { width: width, height: height, children: [_jsx("ellipse", { cx: leafWidth / 2, cy: leafHeight / 2, rx: leafWidth / 2, ry: leafHeight / 2, fill: leftLeafColor, clipPath: `inset(0 50% 0 0)` }), _jsx("ellipse", { cx: leafWidth / 2, cy: leafHeight / 2, rx: leafWidth / 2, ry: leafHeight / 2, fill: rightLeafColor, clipPath: `inset(0 0 0 50%)` }), _jsx("rect", { x: trunkX, y: trunkY, width: trunkWidth, height: trunkHeight, fill: trunkColor })] }));
}
function Sun(props) {
...
export const SequenceMountain = (props) => {
    ...
    const itemBounds = getElementBounds(_jsx(Item, { indexes: [0], data: data, datum: items[0], width: itemWidth }));
    ...
    items.forEach((datum, index) => {
        ...
        decorElements.push(_jsx(Mountain, { colorPrimary: color, x: mountainX, y: mountainY, width: mountainWidth, height: mountainHeight }));
        const itemX = firstMountainLeft + index * (itemWidth + gap);
        labelElements.push(_jsx(Text, { x: itemX, y: labelYPos, width: itemWidth, height: labelHeight, fontSize: 16, fontWeight: "bold", alignHorizontal: "center", alignVertical: "middle", fill: color, backgroundColor: color, backgroundOpacity: 0.5, backgroundRadius: 4, children: String(index + 1).padStart(2, '0') }));
        itemElements.push(_jsx(Item, { indexes: indexes, datum: datum, data: data, x: itemX, y: itemYPos, width: itemWidth }));
    });
    ...
    items.forEach((datum, index) => {
        const indexes = [index];
        const itemX = firstMountainLeft + index * (itemWidth + gap);
        btnElements.push(_jsx(BtnRemove, { indexes: indexes, x: itemX + itemWidth / 2 - btnBounds.width / 2, y: btnY }));
        if (index < items.length - 1) {
            btnElements.push(_jsx(BtnAdd, { indexes: [index + 1], x: itemX + itemWidth + gap / 2 - btnBounds.width / 2, y: btnY }));
        }
    });
    if (n > 0) {
        const firstItemX = firstMountainLeft;
        btnElements.unshift(_jsx(BtnAdd, { indexes: [0], x: firstItemX - gap / 2 - btnBounds.width / 2, y: btnY }));
        const lastItemX = firstMountainLeft + (n - 1) * (itemWidth + gap);
        btnElements.push(_jsx(BtnAdd, { indexes: [n], x: lastItemX + itemWidth + gap / 2 - btnBounds.width / 2, y: btnY }));
        decorElements.push(_jsx(Sun, { x: totalWidth - sunSize - 20, y: -35, width: sunSize, height: sunSize }));
        const treeSizes = [
            'tiny',
            'small',
            'medium',
        ...
        items.forEach((datum, index) => {
            ...
            for (let t = 0; t < treesOnThisMountain; t++) {
                ...
                if (validPosition) {
                    ...
                    decorElements.push(_jsx(Group, { x: treeX, y: treeY, children: Tree(treeSize) }));
                }
            }
        });
        ...
        for (let i = 0; i < cloudCount; i++) {
            ...
            decorElements.push(_jsx(Cloud, { type: cloudType, x: xPos, y: yPos, width: cloudSize.width, height: cloudSize.height }));
        }
    }
    return (_jsxs(FlexLayout, { id: "infographic-container", flexDirection: "column", justifyContent: "center", alignItems: "center", children: [titleContent, _jsxs(Group, { children: [_jsx(Group, { children: decorElements }), _jsx(Group, { children: labelElements }), _jsx(ItemsGroup, { children: itemElements }), _jsx(BtnsGroup, { children: btnElements })] })] }));
};
