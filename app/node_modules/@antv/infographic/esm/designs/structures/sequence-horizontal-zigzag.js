...
export const SequenceHorizontalZigzag = (props) => {
    ...
    items.forEach((item, index) => {
        ...
        if (isEven) {
            ...
            const cardBackground = (_jsx(Rect, { x: cardX, y: cardY, width: cardWidth, height: cardHeight, fill: backgroundColor, rx: 20, ry: 20 }));
            decoElements.push(cardBackground);
        }
        itemElements.push(_jsx(Item, { indexes: indexes, datum: item, data: data, x: cardX + cardPadding, y: cardY + cardPadding, positionH: "center", positionV: "normal" }));
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            decoElements.push(_jsx(Ellipse, { x: dotX, y: dotsY, width: dotSize, height: dotSize, fill: isCurrent ? itemColor : 'transparent', stroke: itemColor, strokeWidth: 2 }));
        }
        btnElements.push(_jsx(BtnRemove, { indexes: indexes, x: cardX + (cardWidth - btnBounds.width) / 2, y: cardY + cardHeight + 10 }));
        if (index === 0) {
            btnElements.push(_jsx(BtnAdd, { indexes: indexes, x: cardX + (cardWidth - btnBounds.width) / 2, y: cardY - btnBounds.height - 10 }));
        }
        else {
            btnElements.push(_jsx(BtnAdd, { indexes: indexes, x: cardX - gap / 2 - btnBounds.width / 2, y: topMargin - btnBounds.height - 10 }));
        }
    });
    if (items.length > 0) {
        const pathSegments = [];
        ...
        const lastColor = getPaletteColor(options, [items.length - 1]) || colorPrimary;
        ...
        const gradientStops = items.map((_, index) => {
            ...
            return _jsx("stop", { offset: `${offset}%`, stopColor: color });
        });
        decoElements.unshift(_jsxs(_Fragment, { children: [_jsx(Defs, { children: _jsx("linearGradient", { id: linearGradientId, x1: startX, y1: startY, x2: endX, y2: endY, gradientUnits: "userSpaceOnUse", children: gradientStops }) }), _jsx(Path, { d: pathD, stroke: `url(#${linearGradientId})`, strokeWidth: 2, fill: "none", width: (items.length - 1) * (cardWidth + gap) + cardWidth + padding * 2, height: cardHeight + 120 })] }));
        decoElements.unshift(_jsx(Ellipse, { x: startX - circleRadius, y: startY - circleRadius, width: circleRadius * 2, height: circleRadius * 2, fill: "transparent", stroke: firstColor, strokeWidth: 2 }));
        decoElements.unshift(_jsx(Ellipse, { x: endX - circleRadius, y: endY - circleRadius, width: circleRadius * 2, height: circleRadius * 2, fill: "transparent", stroke: lastColor, strokeWidth: 2 }));
    }
    if (items.length > 0) {
        const lastIndex = items.length - 1;
        const lastCardX = lastIndex * (cardWidth + gap);
        btnElements.push(_jsx(BtnAdd, { indexes: [items.length], x: lastCardX + cardWidth + (gap - btnBounds.width) / 2, y: topMargin - btnBounds.height - 10 }));
    }
    return (_jsxs(FlexLayout, { id: "infographic-container", flexDirection: "column", justifyContent: "center", alignItems: "center", children: [titleContent, _jsxs(Group, { children: [_jsx(ShapesGroup, { children: decoElements }), _jsx(ItemsGroup, { children: itemElements }), _jsx(BtnsGroup, { children: btnElements })] })] }));
};
