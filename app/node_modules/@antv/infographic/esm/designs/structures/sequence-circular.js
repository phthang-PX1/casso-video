...
export const SequenceCircular = (props) => {
    ...
    // 如果没有数据，显示中心添加按钮
    if (items.length === 0) {
        return (_jsxs(FlexLayout, { id: "infographic-container", flexDirection: "column", justifyContent: "center", alignItems: "center", children: [titleContent, _jsx(Group, { children: _jsx(BtnsGroup, { children: _jsx(BtnAdd, { indexes: [0], x: centerX - 20, y: centerY - 20 }) }) })] }));
    }
    const btnElements = [];
    const itemElements = [];
    const arcElements = [];
    ...
    items.forEach((item, index) => {
        ...
        arcElements.push(_jsx(Path, { d: outerArcPath, fill: lightColor, width: outerRadius * 2, height: outerRadius * 2, "data-element-type": "shape" }));
        // 绘制内层主题色弧形
        ...
        arcElements.push(_jsx(Path, { d: innerArcPath, fill: itemColor, width: outerRadius * 2, height: outerRadius * 2, "data-element-type": "shape" }));
        // 计算图标在弧形中心的位置
        ...
        // 添加外层白色圆形背景
        iconElements.push(_jsx(Ellipse, { x: iconCenterX - iconBgRadius, y: iconCenterY - iconBgRadius, width: iconBgRadius * 2, height: iconBgRadius * 2, fill: "#ffffff", "data-element-type": "shape" }));
        // 添加内层主题色圆形
        iconElements.push(_jsx(Ellipse, { x: iconCenterX - iconRadius, y: iconCenterY - iconRadius, width: iconRadius * 2, height: iconRadius * 2, fill: itemColor, "data-element-type": "shape" }));
        // 添加图标（如果数据项有图标）
        if (item.icon) {
            iconElements.push(_jsx(ItemIcon, { x: iconCenterX - iconSize / 2, y: iconCenterY - iconSize / 2, indexes: indexes, size: iconSize, fill: "#fff" }));
        }
        // 判断Item应该显示在左侧还是右侧
        ...
        itemElements.push(_jsx(Item, { indexes: indexes, datum: item, data: data, x: itemX, y: itemY, positionH: positionH, positionV: positionV }));
        // 添加删除按钮（在Item外侧）
        ...
        const removeBtnY = centerY +
            ...
            btnBounds.height / 2;
        btnElements.push(_jsx(BtnRemove, { indexes: indexes, x: removeBtnX, y: removeBtnY }));
        ...
        btnElements.push(_jsx(BtnAdd, { indexes: [index + 1], x: addBtnX, y: addBtnY }));
    });
    // 添加第一个位置的添加按钮
