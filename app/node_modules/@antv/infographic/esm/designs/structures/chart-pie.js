...
import { getColorPrimary, getPaletteColor, getThemeColors, normalizePercent, } from '../utils/index.js';
...
export const ChartPie = (props) => {
    ...
    finalLabels.forEach((item) => {
        ...
        // --- 绘制连线 ---
        connectorElements.push(_jsx(Path, { d: `M${centerX + p0[0]} ${centerY + p0[1]} 
        ...
        itemElements.push(_jsx(Item, { indexes: [originalIndex], datum: arcDatum.data, data: data, x: itemX, y: itemY, width: labelWidth, height: labelHeight, positionH: isRight ? 'normal' : 'flipped', positionV: "middle", themeColors: getThemeColors({ colorPrimary: color }, options) }));
        // --- 绘制百分比 ---
 ...
 */
export function distributeLabels(items, spacing, minY, maxY) {
    // 避免除零风险
    if (items.length <= 1)
