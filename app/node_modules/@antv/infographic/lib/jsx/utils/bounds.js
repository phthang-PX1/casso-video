"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.getCombinedBounds = getCombinedBounds;
exports.getElementBounds = getElementBounds;
exports.getElementsBounds = getElementsBounds;
const layout_1 = require("../layout");
const element_1 = require("./element");
const is_jsx_element_1 = require("./is-jsx-element");
const is_number_1 = require("./is-number");
const zero = () => ({ x: 0, y: 0, width: 0, height: 0 });
function getCombinedBounds(list) {
    if (!list || list.length === 0)
        return zero();
    let minX = Infinity;
    let minY = Infinity;
    let maxX = -Infinity;
    let maxY = -Infinity;
    for (const bounds of list) {
        const { x, y, width, height } = bounds;
        const x2 = x + width;
        const y2 = y + height;
        if (x < minX)
            minX = x;
        if (y < minY)
            minY = y;
        if (x2 > maxX)
            maxX = x2;
        if (y2 > maxY)
            maxY = y2;
    }
    if (minX === Infinity ||
        minY === Infinity ||
        maxX === -Infinity ||
        maxY === -Infinity) {
        return { x: 0, y: 0, width: 0, height: 0 };
    }
    return {
        x: minX,
        y: minY,
        width: Math.max(0, maxX - minX),
        height: Math.max(0, maxY - minY),
    };
}
function getElementBounds(node) {
    var _a, _b, _c, _d;
    if (!node)
        return zero();
    if (Array.isArray(node)) {
        const elements = (0, element_1.nodeToElements)(node);
        return getElementsBounds(elements);
    }
    if (typeof node !== 'object')
        return zero();
    const { type, props = {} } = node;
    if ((0, layout_1.isLayoutComponent)(node)) {
        const layoutResult = (0, layout_1.performLayout)(node);
        return getElementBounds(layoutResult);
    }
    if (typeof type === 'function') {
        const externalX = (_a = props.x) !== null && _a !== void 0 ? _a : 0;
        const externalY = (_b = props.y) !== null && _b !== void 0 ? _b : 0;
        const externalWidth = props.width;
        const externalHeight = props.height;
        if ((0, is_number_1.isNumber)(externalWidth) && (0, is_number_1.isNumber)(externalHeight)) {
            return {
                x: externalX,
                y: externalY,
                width: externalWidth,
                height: externalHeight,
            };
        }
        const renderedResult = type(props);
        if (!renderedResult) {
            return {
                x: externalX,
                y: externalY,
                width: externalWidth !== null && externalWidth !== void 0 ? externalWidth : 0,
                height: externalHeight !== null && externalHeight !== void 0 ? externalHeight : 0,
            };
        }
        if (Array.isArray(renderedResult)) {
            const elements = (0, element_1.nodeToElements)(renderedResult);
            const innerBounds = getElementsBounds(elements);
            // 如果组件提供了显式尺寸，使用它作为容器边界
            if ((0, is_number_1.isNumber)(externalWidth) && (0, is_number_1.isNumber)(externalHeight)) {
                return {
                    x: externalX,
                    y: externalY,
                    width: externalWidth,
                    height: externalHeight,
                };
            }
            const width = innerBounds.width !== 0 ? innerBounds.width : (externalWidth !== null && externalWidth !== void 0 ? externalWidth : 0);
            const height = innerBounds.height !== 0 ? innerBounds.height : (externalHeight !== null && externalHeight !== void 0 ? externalHeight : 0);
            return {
                x: externalX + innerBounds.x,
                y: externalY + innerBounds.y,
                width,
                height,
            };
        }
        if ((0, is_jsx_element_1.isJSXElement)(renderedResult)) {
            const innerElement = renderedResult;
            const innerBounds = getElementBounds(innerElement);
            if (isPassthrough(innerElement, props)) {
                return innerBounds;
            }
            const innerHasExplicitSize = innerElement.props &&
                (0, is_number_1.isNumber)(innerElement.props.width) &&
                (0, is_number_1.isNumber)(innerElement.props.height);
            const children = getChildrenFromElement(innerElement);
            const jsxChildren = children.flatMap((child) => (0, element_1.nodeToElements)(child));
            const innerHasChildren = jsxChildren.length > 0;
            const innerHasSamePosition = innerElement.props &&
                innerElement.props.x === props.x &&
                innerElement.props.y === props.y;
            if (innerHasExplicitSize) {
                return {
                    x: innerHasSamePosition ? innerBounds.x : externalX + innerBounds.x,
                    y: innerHasSamePosition ? innerBounds.y : externalY + innerBounds.y,
                    width: innerBounds.width,
                    height: innerBounds.height,
                };
            }
            if ((0, is_number_1.isNumber)(externalWidth) && (0, is_number_1.isNumber)(externalHeight)) {
                if (innerHasChildren) {
                    return {
                        x: innerHasSamePosition ? innerBounds.x : externalX,
                        y: innerHasSamePosition ? innerBounds.y : externalY,
                        width: externalWidth,
                        height: externalHeight,
                    };
                }
                else {
                    return {
                        x: innerHasSamePosition ? innerBounds.x : externalX + innerBounds.x,
                        y: innerHasSamePosition ? innerBounds.y : externalY + innerBounds.y,
                        width: externalWidth,
                        height: externalHeight,
                    };
                }
            }
            const width = innerBounds.width !== 0 ? innerBounds.width : (externalWidth !== null && externalWidth !== void 0 ? externalWidth : 0);
            const height = innerBounds.height !== 0 ? innerBounds.height : (externalHeight !== null && externalHeight !== void 0 ? externalHeight : 0);
            return {
                x: innerHasSamePosition ? innerBounds.x : externalX + innerBounds.x,
                y: innerHasSamePosition ? innerBounds.y : externalY + innerBounds.y,
                width,
                height,
            };
        }
        return {
            x: externalX,
            y: externalY,
            width: externalWidth !== null && externalWidth !== void 0 ? externalWidth : 0,
            height: externalHeight !== null && externalHeight !== void 0 ? externalHeight : 0,
        };
    }
    const x = (_c = props.x) !== null && _c !== void 0 ? _c : 0;
    const y = (_d = props.y) !== null && _d !== void 0 ? _d : 0;
    const width = (0, is_number_1.isNumber)(props.width) ? props.width : undefined;
    const height = (0, is_number_1.isNumber)(props.height) ? props.height : undefined;
    if ((0, is_number_1.isNumber)(width) && (0, is_number_1.isNumber)(height)) {
        return { x, y, width, height };
    }
    // 获取子元素来计算边界
    const children = getChildrenFromElement(node);
    const jsxChildren = children.flatMap((child) => (0, element_1.nodeToElements)(child));
    if (jsxChildren.length > 0) {
        const innerBounds = getElementsBounds(jsxChildren);
        return {
            x: x + innerBounds.x,
            y: y + innerBounds.y,
            width: (0, is_number_1.isNumber)(width) ? width : innerBounds.width,
            height: (0, is_number_1.isNumber)(height) ? height : innerBounds.height,
        };
    }
    // 没有子元素且没有显式尺寸
    return {
        x,
        y,
        width: width !== null && width !== void 0 ? width : 0,
        height: height !== null && height !== void 0 ? height : 0,
    };
}
function getElementsBounds(elements) {
    if (!elements || !Array.isArray(elements) || elements.length === 0) {
        return zero();
    }
    const list = [];
    for (const element of elements) {
        const validElements = (0, element_1.nodeToElements)(element);
        for (const validElement of validElements) {
            const bounds = getElementBounds(validElement);
            if (bounds) {
                list.push(bounds);
            }
        }
    }
    return getCombinedBounds(list);
}
function isPassthrough(innerElement, externalProps) {
    if (!(innerElement === null || innerElement === void 0 ? void 0 : innerElement.props) || !externalProps) {
        return false;
    }
    const inner = innerElement.props;
    // 检查位置是否相同
    if ((0, is_number_1.isNumber)(inner.x) &&
        (0, is_number_1.isNumber)(inner.y) &&
        (0, is_number_1.isNumber)(externalProps.x) &&
        (0, is_number_1.isNumber)(externalProps.y)) {
        if (inner.x === externalProps.x && inner.y === externalProps.y) {
            // 如果只有位置相同，还需要检查是否都没有显式尺寸
            const innerHasSize = (0, is_number_1.isNumber)(inner.width) && (0, is_number_1.isNumber)(inner.height);
            const externalHasSize = (0, is_number_1.isNumber)(externalProps.width) && (0, is_number_1.isNumber)(externalProps.height);
            if (!innerHasSize && !externalHasSize) {
                return true;
            }
        }
    }
    // 检查完整的位置和尺寸是否相同
    if ((0, is_number_1.isNumber)(inner.x) &&
        (0, is_number_1.isNumber)(inner.y) &&
        (0, is_number_1.isNumber)(inner.width) &&
        (0, is_number_1.isNumber)(inner.height) &&
        (0, is_number_1.isNumber)(externalProps.x) &&
        (0, is_number_1.isNumber)(externalProps.y) &&
        (0, is_number_1.isNumber)(externalProps.width) &&
        (0, is_number_1.isNumber)(externalProps.height)) {
        return (inner.x === externalProps.x &&
            inner.y === externalProps.y &&
            inner.width === externalProps.width &&
            inner.height === externalProps.height);
    }
    return false;
}
function getChildrenFromElement(element) {
    var _a;
    // React 风格：只从 props.children 获取子元素
    const children = (_a = element.props) === null || _a === void 0 ? void 0 : _a.children;
    if (!children) {
        return [];
    }
    // 标准化为数组
    return Array.isArray(children) ? children : [children];
}
