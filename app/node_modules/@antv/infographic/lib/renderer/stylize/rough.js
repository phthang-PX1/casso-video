...
exports.applyRoughStyle = applyRoughStyle;
...
function applyRoughStyle(node, svg, config) {
    ...
    const rc = roughjs_1.default.svg(svg, {
        options: {
            seed: 1000,
        },
    });
    ...
    (0, utils_1.setAttributes)(volume, {
        opacity: 0.5,
    });
...
function createRoughShape(rc, node, config) {
     ...
     */
    const getShapeAttrs = (node, attrs) => {
        // 获取所有需要的属性
        const allAttrs = (0, utils_1.getAttributes)(node, [
            ...attrs,
            'fill',
            'fill-opacity',
            'stroke',
        ...
        // 处理样式属性
        const { fill, 'fill-opacity': fillOpacity, stroke, 'stroke-opacity': strokeOpacity, 'stroke-width': strokeWidth, 'stroke-dasharray': strokeDasharray, 'stroke-linecap': strokeLinecap, 'stroke-linejoin': strokeLinejoin, opacity, filter, 'clip-path': clipPath, mask, } = allAttrs;
        // RoughJS 支持的属性
        ...
        switch (shapeType) {
            ...
            case 'rect': {
                ...
                if (rx > 0 || ry > 0) {
                    ...
                    const path = createRoundedRectPath(x, y, width, height, effectiveRx, effectiveRy);
